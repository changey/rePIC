...
@synthesize tint,itemArray,nameArray;
...
-(void)readFromCSV:(NSString*)path  TitleAtColumn:(int)tcolumn  DataAtColumn:(int)dcolumn
{
    ...
    nameArray=[[NSArray alloc]initWithObjects:@"Correct",@"Incorrect", nil];
...
-(void)drawBottomTitlesText
{
    ...
    for(int i=0;i<[itemArray count];i++)
    {
        ...
        float percent=([[itemArray objectAtIndex:i] floatValue]/sum)*100;
        ...
        [title setText:[NSString stringWithFormat:@"  %@  (%.0f %@)",[nameArray objectAtIndex:i],percent,@"%"]];
        title.tag=1000+i;
...
- (void)drawRect:(CGRect)rect
{
   ...
   // CGFloat angleArray[c];
    ...
    CGFloat BGLocations[3] = { 0.0, 0.0, 0.0 };
    ...
    for(int i=0;i<[itemArray count];i++)
    {
        ...
        float myAngle=(float)(([[itemArray objectAtIndex:i] intValue])/(float)sum)*(2*3.14); // in radians
